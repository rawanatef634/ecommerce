import express from 'express';
import { listProducts, getProduct, addProduct, removeProduct} from "../controllers/productController.js";
import upload from '../middleware/multer.js'
import adminAuth from '../middleware/adminAuth.js';
const productRoutes = express.Router()

productRoutes.get('/products', listProducts)
productRoutes.post('/addProduct', adminAuth, upload.fields([{name: 'image1', maxCount: 1}, {name: 'image2', maxCount: 1}, {name: 'image3', maxCount: 1}, {name: 'image4', maxCount: 1}]), addProduct)
productRoutes.get('/getProduct', getProduct)
productRoutes.delete('/removeProduct', adminAuth, removeProduct)

export default productRoutes;